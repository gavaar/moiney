...
export function PipeHistoryRow({ event, expanded, onPress, summary, summaryError, onLoadSummary }: Props) {
  ...
  const borderColor = event.pipeType === "boiler" ? colors.secondary : event.pipeType === "feed" ? colors.primary : colors.text;
  ...
  return (
    <View className="flex-row gap-1">
      ...
      <Pressable accessibilityRole="button" accessibilityLabel={deleted ? `${event.name} archived history` : `Open ${event.name}`}
        ...
        className="flex-1 rounded-2xl border bg-surface px-2 py-2 gap-1" style={{ borderColor }}>
        <View className="flex-row items-center gap-1">
          ...
          <Text numberOfLines={1} className="text-text font-bold text-sm flex-1">{event.name}</Text>
          {deleted ? <Text className="text-muted text-xs">Deleted</Text> : null}
          <Text className="text-text text-xs">{dateFormatter.format(event.occurredAt)}</Text>
        </View>
        {deleted ? (
